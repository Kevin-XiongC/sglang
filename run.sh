...
CUDA_VISIBLE_DEVICES="0,1,2,3,4,5,6,7" TORCH_CUDA_ARCH_LIST=9.0 \
    ...
    --model-path /root/kimi --served-model-name kimi --trust-remote-code \
    ...
    --page-size 64 --max-running-requests 128 --mem-fraction-static 0.90 \
    --load-balance-method round_robin --disable-flashinfer
...
CUDA_VISIBLE_DEVICES="0,1,2,3,4,5,6,7" TORCH_CUDA_ARCH_LIST=9.0 \
    python3 -m sglang.launch_server \
    --model-path /root/kimi --served-model-name kimi \
    ...
    --deepep-mode low_latency --tp-size 8 --dp-size 8 --ep-size 8 --moe-dense-tp-size 1 \
    --page-size 64 --watchdog-timeout 1000000 --host 0.0.0.0  --chunked-prefill-size 6144 \
    --prefill-round-robin-balance --load-format dummy --mem-fraction-static 0.65  --disable-flashinfer
